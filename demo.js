...
// Remove Student Scene - Admin Version with Parent Cleanup
const removeStudentScene = new Scenes.BaseScene('remove_student_scene');

removeStudentScene.enter((ctx) => {
    const cancelKeyboard = Markup.keyboard([
        ['❌ Cancel']
    ]).resize();

    ctx.reply('🗑️ Please enter the Student ID to remove from the system:', cancelKeyboard);
});
...
removeStudentScene.on('text', async (ctx) => {
    ...
    if (studentId === '❌ Cancel') {
        ctx.reply('❌ Student removal cancelled.', adminMenu);
    ...
    try {
        ...
        // Get parent info for confirmation
        let parentInfo = 'None';
        if (student.parentId) {
            const parent = await getUserById(student.parentId);
            parentInfo = parent ? `${parent.name} (ID: ${student.parentId})` : `ID: ${student.parentId}`;
        }
        ...
        ctx.replyWithMarkdown(
            ...
            Markup.inlineKeyboard([
                [Markup.button.callback('✅ Yes, Remove Student', 'confirm_remove_student')],
                [Markup.button.callback('❌ No, Cancel', 'cancel_remove_student')]
            ])
        );

    } catch (error) {
        console.error('Error in remove student scene:', error);
...
// Handle confirmation
removeStudentScene.action('confirm_remove_student', async (ctx) => {
    await ctx.answerCbQuery();

    try {
        const { studentId, studentName, className, parentId } = ctx.session.studentToRemove;
        ...
        const relationsResult = await TeacherStudent.deleteMany({ studentId });
        deletedRelations = relationsResult.deletedCount;
        
        // 2. Remove all grades for this student
        const gradesResult = await Grade.deleteMany({ studentId });
        deletedGrades = gradesResult.deletedCount;
        
        // 3. Handle parent unlinking and cleanup
        if (parentId) {
            const parent = await getUserById(parentId);
            if (parent) {
                // Remove this student from parent's studentIds
                parent.studentIds = parent.studentIds.filter(id => id !== studentId);
                
                // If parent has no more students and is not an admin/teacher, delete the user
                if (parent.studentIds.length === 0 && parent.role === 'parent') {
                    await User.deleteOne({ telegramId: parentId });
                    parentDeleted = true;
                } else if (parent.studentIds.length === 0) {
                    // If no students but has another role, downgrade to user
                    parent.role = 'user';
                    await parent.save();
                } else {
                    // Still has other students, just update
                    await parent.save();
                }
                parentHandled = true;
            }
        ...
        let summaryMessage = `✅ *Student successfully removed!*\n\n` +
            `👤 *Student:* ${studentName}\n` +
            `🆔 *ID:* ${studentId}\n` +
            `🏫 *Class:* ${className}\n\n` +
            ...
            `• Parent handled: ${parentHandled ? '✅' : '❌'}\n`;

        // Add parent-specific info
        if (parentHandled) {
            if (parentDeleted) {
                summaryMessage += `• Parent account: 🗑️ Deleted (no more students)\n`;
            } else if (parentId) {
                const parent = await getUserById(parentId);
                if (parent) {
                    summaryMessage += `• Parent account: 🔄 Updated (${parent.studentIds.length} students remain)\n`;
                    summaryMessage += `• Parent role: ${parent.role}\n`;
                }
            }
        }

        summaryMessage += `• Student profile: ✅ Removed\n\n` +
            `*All associated data has been permanently deleted or cleaned.*`;

        ctx.replyWithMarkdown(summaryMessage, adminMenu);

    } catch (error) {
        console.error('Error removing student:', error);
        ctx.reply('❌ An error occurred while removing the student.', adminMenu);
    }
...
// Handle cancellation
removeStudentScene.action('cancel_remove_student', async (ctx) => {
    await ctx.answerCbQuery();
    ctx.reply('❌ Student removal cancelled.', adminMenu);
...
removeStudentScene.hears('❌ Cancel', async (ctx) => {
    ctx.reply('❌ Student removal cancelled.', adminMenu);
